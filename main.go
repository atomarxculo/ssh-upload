...
func main() {
	var server, username, port, password, command, localfile, remotepath string
	...
	flag.StringVar(&command, "command", "", "Comando que va a ejecutar en el servidor.")
	flag.StringVar(&localfile, "localfile", "", "Fichero local a subir")
	flag.StringVar(&remotepath, "remotepath", "/var/tmp/", "Destino del fichero")
	flag.Parse()
	...
	if !pkg.FlagPassed("server") && !pkg.FlagPassed("user") {
		fmt.Println("Tienes que indicar un usuario para poder conectarte.")
		os.Exit(0)
	}
	if pkg.FlagPassed("remotepath") && !pkg.FlagPassed("localfile") {
		fmt.Println("Tienes que elegir un fichero local que subir")
		os.Exit(0)
	}
	if !pkg.FlagPassed("command") {
		command = ""
	}

	if pkg.FlagPassed("server") {
		wg.Add(1)
		go pkg.ConnectSSH(username, port, env, server, command, localfile, remotepath, wg)
		wg.Wait()
	} else {
		...
		for _, serverline := range servertext {
			wg.Add(1)
			go pkg.ConnectSSH(username, port, env, serverline, command, localfile, remotepath, wg)
		}
		wg.Wait()
	}
}
